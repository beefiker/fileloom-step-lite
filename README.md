...
- Extracts `PRODUCT` name, unit hints, `CARTESIAN_POINT`, `VERTEX_POINT`, complex point/vertex records, `DIRECTION`, `VECTOR`, `AXIS2_PLACEMENT_3D`, complex direction/placement records, `LINE`, `POLYLINE`, `POLY_LOOP`, complex line/polyline/loop records, `CIRCLE`, `ELLIPSE`, `PARABOLA`, `HYPERBOLA`, complex conic records, `B_SPLINE_CURVE_WITH_KNOTS`, `BEZIER_CURVE`, `QUASI_UNIFORM_CURVE`, `UNIFORM_CURVE`, complex rational B-spline records, `TRIMMED_CURVE`, `SURFACE_CURVE`, `SEAM_CURVE`, `COMPOSITE_CURVE_SEGMENT`, `COMPOSITE_CURVE`, complex curve wrapper records, `EDGE_CURVE`, and complex edge records
- Emits lightweight line, polyline, closed poly-loop, circle, arc, sampled ellipse, sampled parabola, sampled hyperbola, sampled B-spline, sampled Bezier, sampled rational B-spline, and bounded composite line/polyline/B-spline/trimmed-conic wireframes with 3D bounds for preview adapters
- Counts unsupported curve records instead of flattening them into misleading straight lines
- Emits unreferenced finite `VECTOR`-backed `LINE`, bounded `POLYLINE`, B-spline, composite, `TRIMMED_CURVE`, `SURFACE_CURVE`, `SEAM_CURVE`, full `CIRCLE`, and full `ELLIPSE` curves for wireframe/geometric-set style STEP exports without `EDGE_CURVE` topology
